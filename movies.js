...
// Add's movies in the <main>.
const addMoviesToDom = function(movies) {
    console.log("addMoviesToDom called with movies:", movies);
    // Get's the <ul> by the Id.
    const moviesUl = document.getElementById("movie-list");
    console.log("Movie List <ul>:", moviesUl);
    // Clears the existing movies from the DOM.
    moviesUl.innerHTML = "";
    // Creates <li> elements for every movie in the passed list.
    const listOfMovies = movies.map(movie => {
    const li = document.createElement("li");
    li.innerHTML = '<a href="https://www.imdb.com/title/' + movie.imdbID + '"><img src=' + movie.poster + ' alt=' + movie.title + ' /></a>';
    return li;
    });
    // Add's the <li> elements in the <ul>.
    console.log("listOfMovies elements:", listOfMovies.length);
    listOfMovies.forEach(li => moviesUl.appendChild(li));
    }
    addMoviesToDom(moviesDatabase);


// Functions that get passed in the switch statement of handleOnChangeEvent function in radio-buttons.js.
const filterMoviesByTitle = function (wordInMovie) {
    console.log("filterMoviesByTitle called with argument:", wordInMovie);
    const filteredMovies = moviesDatabase.filter((movie) => movie.title.includes(wordInMovie));
    return filteredMovies;
};

const filterMoviesByYear = function() {
    const latestMovies = moviesDatabase.filter((movie) => {
